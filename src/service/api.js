...
const {nombre,apellido,direccion,telefono,documento,email}=req.body;
if (!nombre || !apellido || !direccion || !telefono ||!documento ||!email){
return res.status(400).json("Todos los datos deben de estar completo")
}
db.query("INSERT INTO clientes (nombre,apellido,direccion,telefono,documento,email) VALUES (?,?,?,?,?,?)",
[nombre,apellido,direccion,telefono,documento,email],
(err,result)=>{
if (err){
console.log(err)
...
db.query("UPDATE clientes SET pdf =? WHERE id=?",
[pdf,id],
(err,result)=>{
if (err){
console.log(err)
